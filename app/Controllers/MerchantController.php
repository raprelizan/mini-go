...
class MerchantController
{
    ...
    public function updatePage(): void
    {
        ...
        foreach ($fields as $field) {
            ...
            $rawValue = $_POST['field_' . $fieldKey] ?? '';
            $value = is_string($rawValue) ? trim($rawValue) : '';
            if (in_array($fieldKey, ['subheadline', 'description'], true)) {
                $value = HtmlSanitizer::sanitize($rawValue);
            }
            if ($value !== '') {
    ...
    public function uploadEditorImage(): void
    {
        ...
        if (empty($_FILES['file']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
            ...
            echo json_encode(['error' => 'الملف غير صالح.']);
            return;
        }
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            http_response_code(400);
            echo json_encode(['error' => 'تعذر رفع الملف.']);
            return;
        }
        ...
        if (!isset($allowed[$mime])) {
            http_response_code(400);
            echo json_encode(['error' => 'صيغة الملف غير مدعومة.']);
            return;
        }

        $uploadDir = __DIR__ . '/../../public/uploads/merchant_' . $merchantId . '/editor';
        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0755, true);
        }

        $safeName = uniqid('editor_', true) . '.' . $allowed[$mime];
        $destination = $uploadDir . '/' . $safeName;
        if (!move_uploaded_file($file['tmp_name'], $destination)) {
            http_response_code(500);
            echo json_encode(['error' => 'تعذر حفظ الملف.']);
            return;
        }

        header('Content-Type: application/json');
        echo json_encode([
            'location' => '/uploads/merchant_' . $merchantId . '/editor/' . $safeName,
        ]);
    }

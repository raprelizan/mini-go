...
class SuperAdminController
{
    ...
    public function pagesIndex(): void
    {
        ...
        foreach ($pages as &$page) {
            $page['customizer_enabled'] = $customizationMap[(int) $page['id']] ?? 0;
        }
        unset($page);
    ...
    public function themeCustomizer(): void
    {
        Auth::requireRole('super_admin');
    ...
    public function saveCustomizer(): void
    {
        Auth::requireRole('super_admin');
        $pageId = (int) ($_POST['page_id'] ?? 0);
        $action = trim($_POST['action'] ?? 'draft');
        $rawJson = trim($_POST['customization_json'] ?? '');
        $payload = json_decode($rawJson, true);

        if ($pageId === 0 || !is_array($payload)) {
            $_SESSION['flash_error'] = 'بيانات التخصيص غير صالحة.';
    ...
    public function toggleCustomizer(): void
    {
        Auth::requireRole('super_admin');
        $pageId = (int) ($_POST['page_id'] ?? 0);
        $enabled = isset($_POST['is_enabled']) ? 1 : 0;
        if ($pageId === 0) {
            $_SESSION['flash_error'] = 'صفحة غير صالحة.';
